...
import { setBankDetails, setRatesToday } from '@store/globalSlice';

const Home = async () => {
  const bankDetailsReq = await fetch('https://mocki.io/v1/a098da89-d72d-40f8-adee-d59c9be911ef');
  const bankDetailsData = await bankDetailsReq.json();
  store.dispatch(setBankDetails(bankDetailsData));

  const ratesTodayReq = await fetch('https://mocki.io/v1/483e7b0f-62a3-40ca-90a0-a75d3457c0b6');
  const ratesTodayData = await ratesTodayReq.json();
